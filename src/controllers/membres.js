/* controllers/membres.js — Annuaire + profil des sociétaires */
const { query, pageClause } = require('../config/database');
const { body, validationResult } = require('express-validator');
const { audit } = require('../services/audit-log');
const { errResponse } = require('../lib/errors');

// Vue publique d'un user. La bio n'est exposée que si bio_public = 1
// (opt-in RGPD), ou pour le propriétaire / un admin.
...
const setActifValidators = [body('actif').isBoolean().withMessage('actif doit être true ou false')];
const setRoleValidators  = [body('role').isIn(['admin','moderateur','membre'])];

// GET /api/membres — annuaire public, paginé
...
// GET /api/membres/me/dashboard — stats perso vs club, ranking, activités
...
// PATCH /api/membres/:id/actif — admin (protège le dernier admin actif)
async function setActif(req, res) {
...
// PATCH /api/membres/:id/role — admin (protège le dernier admin actif)
...
module.exports = {
  userPublic,
  updateValidators, setActifValidators, setRoleValidators,
  list, getOne, update, myDashboard, setActif, setRole,
};
