/* ═════════════════════════════════════════════════════════════════
   controllers/admin.js — Endpoints d'administration (admin role only)
   ═════════════════════════════════════════════════════════════════ */
...
// ── Validators ──────────────────────────────────────────────────
const setStravaConfigValidators = [
...
async function scraperHealth(req, res) {
  ...
  try {
    ...
    res.json({ status, durationMs, eventsCount: events.length, errorsCount: errors.length, fallbacks, errors, log });
  } catch (err) {
...
async function auditPurge(req, res) {
  try {
...
// GET /api/admin/strava-config
async function getStravaConfig(req, res) {
  try {
...
// POST /api/admin/strava-config
async function setStravaConfig(req, res) {
...
// DELETE /api/admin/strava-config
async function deleteStravaConfig(req, res) {
  try {
    await query(
...
async function dashboardLive(req, res) {
  try {
    const [
...
async function getMaintenance(req, res) {
  try {
    ...
    res.json({ enabled: row?.valeur === '1', message: msg?.valeur || '' });
...
async function usersBulk(req, res) {
  const errs = validationResult(req);
  if (!errs.isEmpty()) return res.status(400).json({ error: errs.array()[0].msg });
  try {
...
module.exports = {
  scraperHealth, metrics, auditList, auditPurge,
  getStravaConfig, setStravaConfig, deleteStravaConfig,
  dashboardLive, broadcast, getMaintenance, setMaintenance, usersBulk,
  setStravaConfigValidators, broadcastValidators, setMaintenanceValidators, usersBulkValidators,
};
